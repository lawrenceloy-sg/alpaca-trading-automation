"""
SNAP Wheel Monitor — runs every 15 min during market hours.
Stateless: derives all state from the Alpaca API.
Stage 1: Sell cash-secured put → close at 50% profit → repeat.
Stage 2: If assigned 100 shares → sell covered call 8%+ above cost.
"""
...
SYMBOL          = "SNAP"
MAX_CASH        = 600       # max collateral per contract
DTE_MIN         = 5
DTE_MAX         = 9
DELTA_MIN       = 0.20
DELTA_MAX       = 0.30
CLOSE_PCT       = 0.50      # close at 50% profit
CALL_MIN_ABOVE  = 1.08      # covered call strike >= 8% above cost
RISK_FREE       = 0.045
HV_DAYS         = 30
...
def get_hv(sym, days=HV_DAYS):
    ...
    bars  = resp.get("bars", [])
    if len(bars) < 5:
        print(f"  Insufficient bar data — using default HV 60%")
        return 0.60
...
def find_best_put(sym, S, hv):
    ...
    if not contracts:
        print(f"  No put contracts found for {sym} in DTE {DTE_MIN}-{DTE_MAX}")
        return None
        ...
        if DELTA_MIN <= delta <= DELTA_MAX:
            ...
            if diff < best_diff:
                ...
                best = {"symbol": c["symbol"], "strike": K, "expiration": c["expiration_date"],
                        "dte": dte_days, "delta": round(delta, 4), "theo": round(theo, 4), "cash": cash}
...
# Find short put position (symbol like SNAP260618P00005500, qty negative)
...
# Find equity position (100+ shares of SNAP)
snap_shares = next((p for p in positions
...
if snap_shares:
    qty_shares  = int(float(snap_shares["qty"]))
    cost_basis  = float(snap_shares["avg_entry_price"])
    print(f"STAGE 2: {qty_shares} shares, cost_basis=${cost_basis:.4f}")

    # Check if call already open (order) or already filled (position)
...
if snap_put:
    ...
    exp_str = sym[len(SYMBOL):len(SYMBOL)+6]  # e.g. 260618
    ...
    if profit_pct >= CLOSE_PCT * 100:
        # Check if BTC order already pending
        pending_btc = [o for o in open_orders if o.get("symbol") == sym and o.get("side") == "buy"]
        if pending_btc:
            print(f"  BTC order already pending — waiting for fill")
        else:
...
# ── No position: open new cycle or wait for fill ──────────────────
...
print("No open position — opening new cycle")
hv       = get_hv(SYMBOL)
